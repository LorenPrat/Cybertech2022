...
void frontRight(int rotspeed)
{
    stopMotor(pinMotorA);
    moveMotorBackward(pinMotorB, rotspeed);
}
void backRight(int rotspeed)
{
    stopMotor(pinMotorA);
...
void frontLeft(int rotspeed)
{
    stopMotor(pinMotorB);
    moveMotorBackward(pinMotorA, rotspeed);
}
void backLeft(int rotspeed)
{
    stopMotor(pinMotorB);
    moveMotorForward(pinMotorA, rotspeed);
}
...
void setup()
{
  ...
  pinMode(pin5v2, OUTPUT);
  //pinMode(pinENA, OUTPUT);
  digitalWrite (pin5v,HIGH);
  digitalWrite (pin5v2,HIGH);
  pinMode(CH1, INPUT);
  pinMode(CH2, INPUT);
  pinMode(CH3, INPUT);
  pinMode(CH4, INPUT);
...
void loop()
{
  ...
  if (ch[3]>=1650) {
    ...
    //Serial.println("giro derecha");
      ...
      if (ch[0]>=1700) backRight(vel);
      else if(ch[0]<=1200) backLeft(vel);
      else Forward (veli, veld);
      //Serial.println("marcha atrás");
      }
      else if (ch[1]>=1650){
        if (ch[0]>=1700) frontRight(vel);
        else if(ch[0]<=1200) frontLeft(vel);
        else Back (veli,veld);
